...
export const refreshTokenMiddleware = (IGNORED_ROUTES = []) => async (req, res, next) => {
    ...
    try {
        ...
        if (user.length === 0) return res.status(400).json({ msg: StatusMessage.BAD_REQUEST })
        
        if (refreshToken !== user.refresh_token) return res.status(401).clearCookie('access_token').clearCookie('refresh_token').json({ msg: StatusMessage.REFRESH_TOKEN_REVOKED })
